...
pub enum SemanticError {
    ...
    ForCounterUsedOutOfScope(Token, String),
...
impl Display for SemanticError {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        match self {
            ...
            ForCounterUsedOutOfScope(token, scope_name) => write!(
                f,
                "Semantic error at {}: For counter \"{}\" in \"{}\" is used out of the loop scope.",
                token.location,
                token.lexeme.clone().unwrap(),
                scope_name,
            ),
    ...
    fn get_location(&self) -> Location {
      use SemanticError::*;
        match self {
            ...
            ForCounterUsedOutOfScope(token, _) => token.location,
